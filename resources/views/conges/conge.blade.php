    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="flex items-center justify-between mb-8">
            <h1 class="text-3xl font-extrabold text-gray-900 tracking-tight">
                <span class="bg-clip-text text-transparent bg-gradient-to-r from-indigo-500 to-purple-600">Gestion des Congés</span>
            </h1>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr class="bg-gray-50">
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Employé</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Période</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Jours</th>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="h-10 w-10 flex-shrink-0 bg-indigo-100 rounded-full flex items-center justify-center">
                                            <span class="text-indigo-700 font-medium">{{ strtoupper(substr($conge->user->name, 0, 2)) }}</span>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $conge->user->name }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <div class="text-sm text-gray-900">{{ date('d M Y', strtotime($conge->date_debut)) }}</div>
                                        <div class="text-sm text-gray-500">→ {{ date('d M Y', strtotime($conge->date_fin)) }}</div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900 bg-gray-100 rounded-full px-3 py-1 inline-block">
                                        {{ $conge->jours_demandes }} jours
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    @if ($conge->validation_manager == false && auth()->user()->hasRole('manager') && auth()->user()->departement_id == $conge->user->departement_id)
                                        <div class="flex space-x-2">
                                            ...
                                            <form action="{{ route('conges.rejectManager', $conge->id) }}" method="POST">
                                                @csrf
                                    ...
                                    @elseif ($conge->validation_rh == false && auth()->user()->hasRole('RH') && auth()->user()->departement_id == $conge->user->departement_id)
                                        <div class="flex space-x-2">
                                            ...
                                            <form action="{{ route('conges.rejectRh', $conge->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            ...
            <div class="py-12 flex flex-col items-center justify-center text-center">
                ...
                <p class="mt-1 text-sm text-gray-500">Aucune demande de congé n'a été soumise pour le moment.</p>
            </div>
            @endif
        </div>
    </div>
